...
func TestIntegerLiteralExpression(t *testing.T) {
	...
	integ, ok := stmt.Expression.(*ast.IntegerLiteral)
	if !ok {
		t.Fatalf("exp not *ast.IntegerLiteral. got=%T", stmt.Expression)
	}
	if integ.Value != 5 {
		t.Errorf("integ.Value not %d. got=%d", 5, integ.Value)
	}
	if integ.TokenLiteral() != "5" {
		t.Errorf("integ.TokenLiteral not %s. got=%s", "5", integ.TokenLiteral())
	}
}
...
func testPrefixExpression(t *testing.T, exp ast.Expression, operator string, right interface{}) bool {
	pre, ok := exp.(*ast.PrefixExpression)
	if !ok {
		t.Errorf("exp is not ast.PrefixExpression. got=%T(%s)", exp, exp)
		return false
	}
	if pre.Operator != operator {
		t.Errorf("exp.Operator is not '%s'. got=%q", operator, pre.Operator)
		return false
	}
	if !testLiteralExpression(t, pre.Right, right) {
		return false
	}
	return true
}
...
func TestParsingHashLiteralsWithExpressions(t *testing.T) {
	...
	for key, value := range hash.Pairs {
		...
		if !ok {
			t.Errorf("key not found in tests. got=%s", literal.String())
			continue
		}
		testFunc(value)
	}
